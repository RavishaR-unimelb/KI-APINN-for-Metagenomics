...
print(f'Testing male/female for pinn mod')
...
datanames = ['M3_feces_L5', 'F4_feces_L5']
...
for dataname in datanames:
    ...
    #dataname = 'M3_feces_L6'

    if dataname.startswith('M3'):
        time_steps = 332 #################################
    else:
        time_steps = 130 #################################
    print(f'For {dataname}, timesteps {time_steps}...')
    gnn_type = 'AnchorGNN' #changed ######### check
    ...
    for N in N_list:
        ...
        embeddings_path = f'Embeddings/final_otu_embeddings_titles_and_abstracts_{dataname}_{N_all}.pt' ######### changed
        ...
        use_comm_signature = True ###### changed
        ...
        model = PIGNN(is_aug=is_aug, stats_only=stats_only, include_sent_emb=include_sent_emb, gnn_type=gnn_type, temporal=temporal, 
                    attention_fusion=attention_fusion, use_comm_signature=use_comm_signature, 
                    comm_sig_dim=comm_sig_dim, sent_emb_dim=sent_emb_dim, timepoints=time_steps)
        ...
        for epoch in range(num_epochs):
            print(f'Epoch {epoch+1} for {dataname} ------------------------------')
            model.train()
