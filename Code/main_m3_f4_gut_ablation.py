...
import itertools
import torch
from model_v7_baseline import PIGNN #model
import torch.optim as optim
import matplotlib.pyplot as plt
import random
from helper import plot_true_vs_predicted
from signatures import get_comm_signature, get_comm_signature_all_levels
from paper2.dataset import Dataset
from kg_embeddings import get_kg_based_text_embeddings

print(f'Ablation studies: without reconstruction loss...')
...
# load OTU names
df_raw_path = f'Data/raw/{dataname}.txt'
df_raw = Dataset.from_csv(df_raw_path)
...
for N in N_list:
    ...
    embeddings_path = f'Embeddings/final_otu_embeddings_titles_only_{dataname}_{N_all}.pt'
    results_path = 'Results/'
    is_aug=True
    stats_only=False
    include_sent_emb=True
    temporal = True
    ...
    info = f'{dataname}_{gnn_type}_{N}_{time_steps}_sent_{include_sent_emb}_temp_{temporal}' 
    ...
    # For the PINN
    t = torch.linspace(1, 100, time_steps).unsqueeze(1).requires_grad_(True)
    ...
    community_signature = get_comm_signature(dataname, N, time_steps, ids)
    ...
    model = PIGNN(is_aug=is_aug, stats_only=stats_only, include_sent_emb=include_sent_emb, gnn_type=gnn_type, temporal=temporal, attention_fusion=attention_fusion, use_comm_signature=use_comm_signature, comm_sig_dim=comm_sig_dim, sent_emb_dim=sent_emb_dim)
    ...
    for epoch in range(num_epochs):
        ...
        reconstructed_data, loss, bcd_mean, bcd_all, physics_loss, mse_loss, bcd_per_otu, r2_per_otu, r2_global, sent_recon_loss, r2_per_time, edge_importance, pooled_r2 = model(data, edge_index, t=t, sentence_embeddings=sent_emb, comm_signature=community_signature) 
        print(f'Loss: {loss}')
    ...
    if test:
        # Test - consider all OTUs
        print(f'Testing for {N_test}...')
        model.eval()
        data_test = data_original[:N_test]
        num_nodes_test = data_test.shape[0]
        edges_test = list(itertools.product(range(num_nodes_test), repeat=2))
        edge_index_test = torch.tensor(edges_test, dtype=torch.long).t().contiguous() 
        sent_emb_test = sent_emb_original[:N_test]
        reconstructed_data, loss, bcd_mean, bcd_all, physics_loss, mse_loss, bcd_per_otu, r2_per_otu, r2_global, r2_per_time, _, _ = model(data_test, edge_index_test, t=t, sentence_embeddings=sent_emb_test) 
        print(f'Loss: {loss}')
        print(f'r2_per_otu={r2_per_otu}')
        print(f'r2_global={r2_global}')
