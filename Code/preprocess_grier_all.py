...
for pid, df_pid in participant_dfs.items():
    ...
    otu_list = df3.df.index
    print(f'length of otu_list:{len(otu_list)}')
    ...
    final_data[pid] = (otu_list, data_norm)

print(f'final_data: {final_data}')   
torch.save(final_data, f'Embeddings/preprocessed_data_global_{name}_train_test_{time_points}.pt')
