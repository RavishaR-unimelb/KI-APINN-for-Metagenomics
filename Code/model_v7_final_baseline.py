...
# Changing the encoder architectures
...
class TimeAwareMLP_LSTM(nn.Module):
    def __init__(self, input_dim=1, hidden_dim=64, output_dim=64, num_layers=1, dropout=0.0):
        super().__init__()
    ...
    def forward(self, x):  # x shape: (batch_size, seq_len, input_dim)
        ...
        # Option 1: use last time step output (most common)
        last_out = out[:, -1, :]          # shape: (batch_size, hidden_dim)
        
        # Option 2 (equivalent for single-layer LSTM):
        # last_out = h_n[-1]              # last layer's hidden state
        
        out = self.fc(last_out)           # shape: (batch_size, output_dim)
        return out
...
class GraphNN(torch.nn.Module):
    def __init__(self, gnn_type, in_gcn=64*2, out_gcn=32, out_mlp=1, heads=8):
        ...
        self.mlp = nn.Sequential(
            nn.Linear(2 * self.out_, 100),  # 2*F → hidden
            #nn.ReLU(),
            nn.Tanh(),
...
class PIGNN(torch.nn.Module):
    def __init__(self, num_nodes=15, heads=8, timepoints=100, is_aug=True, stats_only=False, include_sent_emb=False, gnn_type='GAT', temporal=False, attention_fusion=False, use_comm_signature=False, comm_sig_dim=1, sent_emb_dim=384, mono_species=False):
        super(PIGNN, self).__init__()
        # Encoder
        #self.encoder = Encoder_RNN(num_otus=num_nodes)
        #self.encoder = MLPEncoder()
        #self.encoder = MLPEncoderStats()
        self.mono_species = mono_species

        self.include_sent_emb = include_sent_emb
        self.use_comm_signature = use_comm_signature
        self.encoder = Encoder_PINN(is_aug=is_aug, stats_only=stats_only, include_sent_emb=include_sent_emb, temporal=temporal, attention_fusion=attention_fusion, time_steps=timepoints, use_comm_signature=self.use_comm_signature, sent_emb_dim=sent_emb_dim)
        ...
        if temporal:
            if include_sent_emb:
                self.gnn = GraphNN(in_gcn=(64*3)+gnn_add, out_gcn=self.decoder_input_size, gnn_type=gnn_type, heads=heads)
            else:
                self.gnn = GraphNN(in_gcn=(64*2)+gnn_add, out_gcn=self.decoder_input_size, gnn_type=gnn_type, heads=heads)
        else:
            if include_sent_emb:
                self.gnn = GraphNN(in_gcn=(64*2)+gnn_add, out_gcn=self.decoder_input_size, gnn_type=gnn_type, heads=heads)
            else:
                self.gnn = GraphNN(in_gcn=64+gnn_add, out_gcn=self.decoder_input_size, gnn_type=gnn_type, heads=heads)
        ...
        if gnn_type == 'GAT':
            ...
            self.sentence_decoder = SentDecoder(heads=heads, in_decoder=self.decoder_input_size, out_decoder=sent_emb_dim)
        else:
            self.decoder = Decoder(out_decoder=timepoints, in_decoder=self.decoder_input_size)
            self.sentence_decoder = SentDecoder(in_decoder=self.decoder_input_size, out_decoder=sent_emb_dim)
        ...
        self.sent_resize_for_r = nn.Linear(sent_emb_dim, num_out)
    ...
    def forward(self, x, edge_index, t, sentence_embeddings=None, growth_rates=None, comm_signature=None):
        # Encoder
        ...
        ########################################################################################
        '''
        x_out, edge_linear, att_mat = self.gnn(encoded, edge_index)
        ...
        if not self.include_sent_emb:
            ...
            loss = (self.coeffs['mse_coeff']*loss_1) + (self.coeffs['bcd_coeff']*loss_2)
            ...
            loss = (self.coeffs['mse_coeff']*loss_1) + (self.coeffs['bcd_coeff']*loss_2) + (self.coeffs['sent_coeff']*loss_4)
        ...
        if self.mono_species:
            bcd_mean, bcd_all = self.differentiable_bcd_standard(x_reconstructed_norm.detach(), x.detach())
            print(f'For monospecies samples in train/test, the BCD acc (updated) is {1-bcd_mean}')
        else:
            bcd_all, bcd_mean = self.mean_bcd_per_timestep_standard(x_reconstructed_norm.detach(), x.detach()) #changed
            #print(f'bcd per timestep={bcd_all}')
            print(f'updated bcd_mean={bcd_mean}, loss_2={loss_2}')
        ...
        pooled_r2 = self.evaluate_pooled_r2(x_reconstructed_norm.detach(), x.detach())
        comp_r2 = self.evaluate_compositional_r2(x_reconstructed_norm.detach(), x.detach())
        print(f'r2_global:{r2_global}, pooled_r2:{pooled_r2}, comp_r2:{comp_r2}')
        ...
        return x_reconstructed_norm, loss, bcd_mean, bcd_all, loss_3, loss_1, bcd_per_otu, r2_per_otu, r2_global, loss_4, r2_per_time, edge_linear, pooled_r2, comp_r2
    ...
    def differentiable_bcd_standard(self, x_reconstructed, x_real, eps=1e-8, per_timestep_normalize=True):
        """
        Differentiable Bray-Curtis Dissimilarity Loss.
        
        Inputs:
            x_reconstructed: torch.Tensor [num_otus, num_timesteps]
            x_real: torch.Tensor [num_otus, num_timesteps]
            per_timestep_normalize (bool): 
                If True, normalize abundances per time step (standard in metagenomics)
        Returns:
            bcd_per_time: Tensor of BCD values per time point [num_timesteps]
            bcd_mean: Mean BCD across all time points (scalar)
        """

        # Ensure non-negative values
        x_real = torch.clamp(x_real, min=0)
        x_reconstructed = torch.clamp(x_reconstructed, min=0)

        if per_timestep_normalize:
            # Normalize each column (timestep) to sum to 1
            x_real = x_real / (x_real.sum(dim=0, keepdim=True) + eps)
            x_reconstructed = x_reconstructed / (x_reconstructed.sum(dim=0, keepdim=True) + eps)

        # Compute Bray-Curtis Dissimilarity per timepoint
        numerator = torch.sum(torch.abs(x_real - x_reconstructed), dim=0)
        denominator = torch.sum(x_real + x_reconstructed, dim=0) + eps
        bcd_per_time = numerator / denominator

        # Mean BCD across all timepoints
        bcd_mean = bcd_per_time.mean()

        return bcd_mean, bcd_per_time

    def evaluate_compositional_r2(self, x_reconstructed, x_real, eps=1e-8):
        """
        ...
        for t in range(x_real.shape[1]):
            if np.all(x_real[:, t] == x_real[0, t]):
                print(f"Column {t} has zero variance!")
    ...
    def evaluate_pooled_r2(self, x_reconstructed, x_real):
        """
        Computes R² per OTU (comparing time series), and the mean R².
        Input shape: (num_otus, num_timesteps)
        """
